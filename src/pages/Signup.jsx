...
const Signup = () => {
    ...
    const updateDateOfBirth = (day, month, year) => {
        if(day && month && year){
            //convert month name to number (01,01...,12)
            const monthMap = {
    ...
    const handleSubmit = async(e)=>{
       ...
       try {
         const response = await axios.post(`https://fb-clone-backend-dun.vercel.app/api/v1/auth/register`, formData, {
            headers: {
    ...
    return (
        <div className='h-screen flex flex-col gap-7 items-center justify-center py-10'>
            ...
            <div className='bg-white dark:bg-[#262829]  p-3 rounded-lg shadow-md w-full md:max-w-[450px] mx-1 md:mx-0'>
                ...
                <form onSubmit={handleSubmit} className='space-y-2 flex flex-col items-center'>
                    ...
                    <div className='w-full'>
                        ...
                        <div className='flex gap-3 mt-1'>
                            {["Female", "Male", "Custom"].map((gender) => (
                                <label key={gender} className='flex items-center justify-between border border-gray-300 rounded px-3 py-2 w-full cursor-pointer'>
                                    {gender}
                                    <input type="radio" 
                                    name='gender' 
                                    value={gender} 
                    ...
                    <p className='text-xs text-gray-600 dark:text-gray-200 mt-2'>
                        ...
                        <Link className='text-blue-800 dark:text-blue-500 hover:underline'>Learn more</Link>
                    </p>
                    <p className='text-xs text-gray-600 dark:text-gray-200 mt-1'>
                        By clicking Sign Up, you agree to our{" "}
                        <Link className='text-blue-800 dark:text-blue-500 hover:underline'>Terms</Link>{" "}
                        <Link className='text-blue-800 dark:text-blue-500 hover:underline'>Privacy Policy</Link> and{" "}
                        <Link className='text-blue-800 dark:text-blue-500 hover:underline'>Cookies Policy</Link>
                        You may receive SMS notifications from us and can opt out at any time.
                    </p>
                    <button type='submit' className='bg-green-600 text-white text-lg font-bold py-1 w-[200px] px-7 rounded hover:bg-green-700 mt-4'>Sign Up</button>
                    <Link to={'/login'}>
                    <p className='text-blue-600 dark:text-blue-500 hover:text-blue-700 text-lg my-2 cursor-pointer'>Already have an account</p></Link>
                </form>
            </div>
        </div>
    )
