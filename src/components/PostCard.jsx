...
import { Image, MessageSquare, Smile, ThumbsUp, Video, X } from 'lucide-react';
...
const PostCard = ({ post }) => {
  ...
  const likeOrDislikeHandler = async () => {
    try {
      ...
      const res = await axios.get(`https://fb-clone-backend-dun.vercel.app/api/v1/post/${post._id}/${action}`, { withCredentials: true })
      if (res.data.success) {
  ...
  const deleteHandler = async (id) => {
    try {
      const res = await axios.delete(`https://fb-clone-backend-dun.vercel.app/api/v1/post/delete/${id}`, { withCredentials: true })
      if (res.data.success) {
        toast.success(res.data.message)
      }
    } catch (error) {
  ...
  const onSubmitHandler = async (id) => {
    ...
    try {
      const res = await axios.put(`https://fb-clone-backend-dun.vercel.app/api/v1/post/update-post/${id}`, formData, {
        headers: {
  ...
  return (
    <div className='bg-white dark:bg-[#262829] p-4 rounded-lg md:w-[600px] mt-5'>
      <div className='flex justify-between items-center mb-2'>
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogContent className="sm:max-w-[500px] dark:bg-[#262829]">
            ...
            <DialogFooter>
              <Button onClick={()=>onSubmitHandler(post._id)} type="submit" className='w-full bg-[#0866ff] hover:bg-[#0867ffce]'>Post</Button>
            </DialogFooter>
          </DialogContent>
